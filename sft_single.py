...
DEFAULT_MODEL_PATH = "maanka2/MOSS-TTS-Nano-100M"
DEFAULT_CODEC_PATH = "models/MOSS-Audio-Tokenizer-Nano"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--target-repo", type=str, default="")
...
def main() -> None:
    ...
    if accelerator.is_main_process and args.hf_token and args.target_repo:
        ...
        try:
            ...
            api.upload_folder(
                folder_path=str(output_root / "checkpoint-last"),
                repo_id=args.target_repo,
                repo_type="model",
                token=args.hf_token
            )
            accelerator.print(f"[{format_timestamp()}] [sft] uploaded final checkpoint to HF")
        except Exception as e:
            accelerator.print(f"[{format_timestamp()}] [sft] HF upload failed: {e}")
    accelerator.print(
